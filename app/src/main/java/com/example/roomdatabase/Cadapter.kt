...
class Cadapter(private val context: Context, private val dataList: List<User>?) :
    ...
    override fun onBindViewHolder(holder: CustomViewHolder, position: Int) {
        ...
        holder.country.setText(dataList?.get(position)?.country)
        holder.city.setText(dataList?.get(position)?.city)

         holder.delete.setOnClickListener {
             dataList.let { list->
                 db.getIns(holder.itemView.context)?.Cdao()?.Delete(list.get(position))
